...
public class Admin_logon extends HttpServlet
{
	...
	//param.hashTable.clear();//make empty hashTable
     ...
     public void doGet(HttpServletRequest req,HttpServletResponse res)throws ServletException,IOException
     {
			...
			try{
						...
						System.out.println((String)hashTable.get("logon_id"));


						String logon_id=(String)hashTable.get("logon_id");
						String password=(String)hashTable.get("password");
						check_for_right_Administrator(param,logon_id,password);
	...
	public void check_for_right_Administrator (c_Parameters param,String logon_id,String password)throws ServletException,IOException
		{
				...
				try{		
							...
							System.out.println( password.length() );System.out.print(logon_id.length());

							
							if(logon_id==null || password==null || logon_id.length()==0 || password.length()==0)
							{
													...
													String sql="select login_id,password from  Login where login_id='"+logon_id+"'  and password='"+password+ "'	";
													PreparedStatement st=param.con.prepareStatement(sql);
																							//								st.setString(1,logon_id);
...
//															if(Session.isNew())
//															{
																Session.putValue("login_id",(String)hashTable.get("logon_id"));
